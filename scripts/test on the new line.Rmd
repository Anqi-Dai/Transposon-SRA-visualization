...
res <- map(c('Control','Huntingtons'), function(statusGroup){
  ret = prepare_table_for_plotting(TE_name, Num, statusGroup) %>%
  ggplot() + 
      geom_line(aes(x=bin,
                  y=Median,
                  group=Strand,
...
ggarrange(res[[1]], res[[2]], ncol = 2) +
  ggsave(str_glue('../figs/0000000.{TE_name} {Num} line.jpg'), width = 40, height = 12, dpi = 300)
