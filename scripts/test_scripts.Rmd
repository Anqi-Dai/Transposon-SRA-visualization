...
```{r}
# to arrange them in one pipeline
# the fake data in the real data format
...
pos <- read_csv('../data/test_data_pos.csv') %>%
  ...
  rename(Sample =rowname) %>%
  gather(key = bin , value = cnt, b0:b99) %>%
  mutate(Group = if_else(grepl(pattern = '^C', Sample), 'Control', 'Disease'))
...
pos_reg <- apply(pos_wide, 1, function(Row){
  ret = summary(glm.nb(as.numeric(Row) ~ pheno))[[12]][2,4]  
  return(ret)
}) %>% 
  as.data.frame %>% 
